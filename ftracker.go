...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	spentCalories := (18 * meanSpeed * 1.79) * weight / mInKm * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeed := meanSpeed(action, duration)
	meanSpeedInSeconds := meanSpeed * 1000 / 3600
	spentCalories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedInSeconds, 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return spentCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	spentCalories := (meanSpeed + 1.1) * 2 * weight * duration
	return spentCalories
}
